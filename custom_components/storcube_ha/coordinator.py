...
class StorCubeDataUpdateCoordinator(DataUpdateCoordinator):
            ...
            try:
                if scene_data:
                    ...
                    if equip_id:
                        ...
                        # Certains firmwares (ex. S1000-PRO) ne poussent jamais
                        # l'esclave via WebSocket : sans ça, son appareil n'est
                        # jamais créé. La scène REST liste pourtant tous les
                        # equipId du stack dans "equipIds".
                        for member_id in scene_data.get("equipIds") or []:
                            if member_id and member_id != equip_id:
                                self._async_register_device(
                                    member_id,
                                    {"equipType": scene_data.get("equipType")},
                                )
    ...
    async def _async_handle_ws_message(self, message) -> None:
        ...
        if not batteries:
            _LOGGER.debug("Trame WebSocket sans batterie : %s", data)
            return
